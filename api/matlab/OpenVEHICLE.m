...
function result = OpenVEHICLE()
    ...
    result = vehname; % ADDED: return vehname so that we can access .mat from api.py
    delete(vehname+".log") ;
    diary(vehname+".log") ;
    disp([...
    ...
    %% Plot DO IT FOR THE PLOT HOLAY MOLAY
    ...
    % % saving figure
    % savefig(vehname+".fig")
    % % concat datetime to differentiate png files
    % % timestamp = string(datetime('now'), 'yyyymmdd_HHMMSS');
    % % filename = vehname + ".png";
